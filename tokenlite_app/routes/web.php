...
Route::get( '/auth0logout', 'Auth\Auth0IndexController@logout' )->name( 'auth0logout' );
...
// Multi-campaign: Setting the Token to sell
Route::get('/token/{tokensymbol?}', 'User\TokenSelectController@select')->where("tokenId","[A-Z]")->name('token');
...
Route::prefix('user')->middleware(['auth', 'user', 'verify_user', 'g2fa'])->name('user.')->group(function () {
    Route::get('/selfcertify', 'User\SelfCertifyController@selfcertify')->name('selfcertify');
    Route::post('/selfcertify1', 'User\SelfCertifyController@selfcertify1')->name('selfcertify1');
    Route::post('/selfcertify2', 'User\SelfCertifyController@selfcertify2')->name('selfcertify2');
    Route::post('/selfcertify3', 'User\SelfCertifyController@selfcertify3')->name('selfcertify3');
    Route::post('/selfcertify4', 'User\SelfCertifyController@selfcertify4')->name('selfcertify4');

    Route::group(['middleware' => 'selfcertify'], function(){
        ...
        Route::get('/contribute', 'User\TokenController@index')->middleware('select_token')->name('token');
        Route::get('/contribute/cancel/{gateway?}', 'User\TokenController@payment_cancel')->name('payment.cancel');
        Route::get('/transactions', 'User\TransactionController@index')->middleware('select_token')->name('transactions');
        Route::get('/token-list', 'User\TokenListController@index')->name('token-list');
        Route::get('/signage', 'User\SignController@index')->middleware('select_token')->name('signage');
        Route::post('/signage', 'User\SignController@post')->middleware('select_token')->name('signage_post');
        Route::get('/signage/application', 'User\SignController@application')->name('signage.application');
        Route::get('/signage/application/view', 'User\SignController@view')->name('signage.application.view');
        ...
        // My Token v1.1.2
        Route::get('/account/balance', 'User\UserController@mytoken_balance')->middleware('select_token')->name('token.balance');

        Route::get('/listing', 'User\ListingController@index')->name('listing');
        // Route::view('/listing', 'user.listing');
    });
    ...
    Route::name('ajax.')->prefix('ajax')->group(function () {
        ...
        Route::post('/account/selfcert', 'User\UserController@account_selfcert')->name('account.selfcert');
...
Route::prefix('admin')->middleware(['auth', 'admin', 'g2fa', 'ico'])->name('admin.')->group(function () {
    ...
    Route::get('/stages/{tokenid?}', 'Admin\IcoController@index')->middleware('ico')->name('stages');
    Route::get('/stages/{tokenid}/{id}', 'Admin\IcoController@edit_stage')->middleware('ico')->name('stages.edit');
    // Route::get('/stages/settings/{tokenid?}', 'Admin\IcoController@settings')->middleware('ico')->name('stages.settings');
    Route::get('/tokens/settings', 'Admin\TokenSetupController@index')->name('tokens.settings');
    Route::post('/tokens/updateToken', 'Admin\TokenSetupController@updateToken')->name('tokens.display');
    Route::post('/tokens/editToken/{id}', 'Admin\TokenSetupController@editToken')->name('tokens.display');
    Route::get('/tokens/{id?}/{type?}', 'Admin\TokenSetupController@show')->name('tokens.view');
    ...
    Route::get('/updateusertype/{id}','Admin\UsersController@update_usertype')->name('updateRole');
    ...
    Route::name('ajax.')->prefix('ajax')->middleware(['ico'])->group(function () {
        ...
        Route::post('/tokens/upload', 'Admin\TokenSetupController@upload_zone')->name('tokens.upload')->middleware('demo_user');
        Route::post('/tokens/view', 'Admin\TokenSetupController@status')->name('tokens.view')->middleware('demo_user');
        Route::post('/tokens/insert', 'Admin\TokenSetupController@store')->middleware(['super_admin', 'demo_user'])->name('tokens.add');
