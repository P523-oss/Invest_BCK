...
<div class="page-ath-form">
    ...
    <form class="register-form" method="POST" action="{{ route('register2') }}" id="register2">
        REGISTER2
        @csrf
        @include('layouts.messages')
        @if(! is_maintenance() && application_installed(true) && ($check_users == 0) )
        <div class="alert alert-info-alt">
            Please register first your Super Admin account with administration privilege.
        </div>
        @endif
        ...
        <div class="input-item">
            <label for="user_type_id">
                Member type:</label>
            <select name="user_type_id" id="user_type_id" class="input-bordered">

                @foreach($user_types as $utype)

                <option value="{{ ($utype->id) }}"> {{ $utype->description}}</option>
        ...
        @endif
        <button type="submit" class="btn btn-primary btn-block">{{ ( application_installed(true) && ($check_users == 0) ) ? __('Complete Installation') : __('Continue') }}</button>
    </form>
