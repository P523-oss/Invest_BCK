...
$stage=active_stage();
$min_token = ($minimum) ? $minimum : $stage->min_purchase;
$token_id=$token->id;
$stage_id=$stage->id;

$sold_token = ($stage->soldout + $stage->soldlock);
$have_token = ($stage->total_tokens - $sold_token);
$sales_ended = (($sold_token >= $stage->total_tokens) || ($have_token < $min_token)) ? true : false;
...
<div class="alert alert-dismissible fade show alert-info" role="alert">
    ...
    {{ __('Sales Start at') }} - {{ _date($stage->start_date) }}
</div>
@endif
<div class="content-area card">
    <div class="card-innr">
        <form action="javascript:void(0)" method="post" class="token-purchase">
            ...
            <input type="hidden" id="data_token_id" value="{{ $token_id }}">
            <input type="hidden" id="data_stage_id" value="{{ $stage_id }}">
        </form>
    </div> {{-- .card-innr --}}
</div> {{-- .content-area --}}
@push('sidebar')
<div class="aside sidebar-right col-lg-4">
    ...
    {{-- Comented out, requested by Manindra and Zafer --}}
