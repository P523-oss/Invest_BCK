...
if (!function_exists('token_symbol')) {
    function token_symbol($token_id=null)
    {
        $token_symbol="";
        if (is_null($token_id)){
            // return IcoHandler::get_token_settings('symbol'); 
            $token_symbol= session()->has('currentsymbol')? session('currentsymbol') : "NOSYMBOL";
        } else {
            $token_symbol= Token::getSymbol($token_id);
        }
        return $token_symbol;
    }
}
...
function notify_admin($tnx, $name = '')
{
    ...
    $admins =User::join('user_metas', 'users.id', '=', 'user_metas.userId')->whereIn('users.role', ['admin','supervisor'] )->where(['users.status' => 'active', 'user_metas.notify_admin' => 1])->select('users.*')->get();
...
if (!function_exists('token_calc')) {
    function token_calc( $amount, $output = 'total', $token_id=null)
    {
        if (empty($amount)) {
            return 0;
        }

        $res = new TokenCalculate($token_id);
...
function html_table($data = array(), $titles="", $style)
{
    $rows = array();
    $title="";
    $text ="";
    ...
    if ($style=="list"){
        $totalContr = 0;
        foreach ($data as $row) {
            $text .= $row["tokenBalance"] ." ". $row["token_id"] . "<br />";
            $totalContr += (float) $row["contributed"];
        }
        if ($totalContr==0)
            $text = "0";
        else 
            $text .= "Total Contributed = ". $totalContr;
        $return = $text;
    } else{
        if ($titles && $style !="list"){
            foreach ($titles as $ttl) {
                $titlecells[] = "<th class='text-center'>{$ttl}</th>";
            }
            $title = "<thead><tr>" . implode('', $titlecells) . "</tr></thead>";
        }
        foreach ($data as $row) {
            $cells = array();
            foreach ($row as $cell) {
                $cells[] = "<td class='text-right'>{$cell}</td>";
            }
            $rows[] = "<tbody><tr>" . implode('', $cells) . "</tr></tbody>";
        }
        $return = "<div class='table-responsive'><table class='table table-striped table-bordered tbl_data'>" . $title. implode('', $rows) . "</table></div>";
    }
    return $return;
}
...
if (!function_exists('active_stage')) {
    function active_stage($token_id=0, $id = '' )
    {
        if (!$token_id){
        ...
        $actived_stage = Token::get_actived_stage($token_id);
        if ($actived_stage != '' && is_numeric($actived_stage)) {
            // $stage = IcoStage::where('status', '!=', 'deleted')->where('token_id', $token_id)->where('id', get_setting('actived_stage'))->first();
            $stage = IcoStage::where('status', '!=', 'deleted')->where('token_id', $token_id)->where('id', $actived_stage)->first();
            if (!$stage) {
                $stage = IcoStage::where('status', '!=', 'deleted')->where('token_id', $token_id)->orderBy('id', 'DESC')->first();
            }
        } elseif ($id != '') {
            $stage = IcoStage::where('status', '!=', 'deleted')->where('token_id', $token_id)->find($id);
        } else {
            $stage = IcoStage::where('status', '!=', 'deleted')->where('token_id', $token_id)->first();
        }
...
if (!function_exists('get_base_bonus')) {
    function get_base_bonus($id, $type=null, $token_id = null) {
        $tc = new TokenCalculate($token_id);
        $bonus = NULL;
        if(!empty($id)){
...
if (!function_exists('current_price')) {
    function current_price($type=null, $token_id = null) {
        $tc = new TokenCalculate($token_id);
...
if (!function_exists('approved_by')) {
    function approved_by($data)
    {
        ...
        if (isset($data->id)) {
            ...
            $user = User::find($id);
            if ($user) {
                $return = ($user->role == 'admin' || $user->role == 'supervisor') ? $user->name.' ('.ucfirst($user->role).')' : 'Contributor';
...
if (!function_exists('ico_stage_progress')) {
    function ico_stage_progress($type, $in_currency='token', $istage=null) {
        ...
        $symbol = Token::getSymbol($stage->token_id);
        $cur = ($in_currency=='token') ? $symbol : strtoupper($in_currency);
        if($type == 'soft'){
...
if (!function_exists('tokens')) {
    function tokens($tokenSymbol=null)
    {
        if(!$tokenSymbol){
            $tokenSymbol = 'BCX';
        }
        ...
        $token_id = Token::where('token_symbol',$tokenSymbol)->first();
        $tokenSetting = TokenSettings::where('token_id',$token_id->id)->first();
        
        if(!$tokenSetting){
         return '/images/logo2x.png';   
        }
        $path = '/images/logo/'.$tokenSetting->token_logo;
        return $path;
    }
}

if (!function_exists('bgColor')) {
    function bgColor($tokenSymbol=null)
    {
        if(!$tokenSymbol){
            $tokenSymbol = 'BCX';
        }
        ...
        return 'background:#'.$tokenSetting->bg_color.' !important';
    }
}