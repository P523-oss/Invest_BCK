...
class TokenCalculate
{
    ...
    {
        if (is_null($token_id)){
            $token_symbol= session()->has('currentsymbol')? session('currentsymbol') : "";
            $token = Token::where('token_symbol', $token_symbol)->get()->first();
            if (is_null($token)){
                $token = Token::get()->first();
                session(['currentsymbol' => $token->token_symbol]);
                session(['currenttoken' => serialize($token)]);
            }
            $token_id = $token->id;
        } else {
            $token = Token::where('id', $token_id)->get()->first();
        }

        $actived_stage =null;
        if ($token){
            // $actived_stage = Token::get_actived_stage($token_id);
            $actived_stage = $token->actived_stage;
        }
        if ($actived_stage) {
            $this->stage = IcoStage::where('status', '!=', 'deleted')->where('id', $actived_stage)->first();
            if (!$this->stage) {
                $this->stage = IcoStage::where('status', '!=', 'deleted')->orderBy('token_id', $token_id)->first();
            }
        // } else {
    ...
    public function get_current_price($attr = '')
    {
        ...
        if (!empty($data)) {
            foreach ($data as $tire => $tire_opt) {
                if(!property_exists ($tire_opt, 'status')) die("Warning Token Sale Metas lacks info ... ");
    ...
    public function calc_token($token, $output = 'total')
    {
        ...
        $activeCurrency =Setting::active_currency(); 
        foreach ($activeCurrency as $name => $rate) {
