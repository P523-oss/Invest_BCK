...
class UserPanel
{
    ...
    public static function user_info($data = null, $atttr = '')
    {
        ...
        <div class="user-status-balance">' . ($user->tableUserBalance())  . '</div>
        </div>';
    ...
    public static function user_balance($data = null, $atttr = '')
    {
        ...
        <h6 class="user-status-title">' . __('Token Balance') . '</h6>
        <div class="small user-status-balanceNOT ">' . ($user->tableUserBalance()) . '</div>
        </div>';
    ...
    {
        $token = unserialize(session('currenttoken'));
        $user = auth()->user();
        ...
        $ver_icon = ($vers == 'side') ? '<div class="token-balance-icon"><img src="' . asset('images/symbol/'. $token->logo) . '" alt=""></div>' : '';
        ...
        <span class="lead">' .($user->getBalancesByToken($token->id)['tokenBalance']) . ' <span>' . $token->token_symbol . ' <em class="fas fa-info-circle fs-11" data-toggle="tooltip" data-placement="right" title="'.__('Equivalent to'). ' ' .token_price($user->getBalancesByToken($token->id)['tokenBalance'], base_currency()).' '.base_currency(true).'"></em></span></span>
        </div>
    ...
    public static function user_token_block($data = '', $atttr = '')
    {
        ...
        // $_SYM = token_symbol();
        $_SYM = $token->token_symbol;
        $base_currency = base_currency();
        $token_1price = token_calc(1, 'price', $token->id)->$base_currency;
    ...
    public static function user_account_status($data = null, $atttr = '')
    {
        ...
        $g_id = ($id) ? ' id="' . $id . '"' : '';
        $g_cls = ($class) ? css_class($class) : '';
        $self_cert_status = '<li><a href="javascript:void(0)" class="btn btn-xs btn-auto btn-warning">Self Cert Pending</a></li>';
        ...
        if(!is_kyc_hide()) {
            if (!$user->selfCertified()) {
                $kyc_staus = '<li><a href="' . route('user.selfcertify') . '" class="btn btn-xs btn-auto btn-warning">' . __('Self Certify') . '</a></li>';
            } elseif (isset($user->kyc_info->status) && $user->kyc_info->status == 'approved') {
            ...
            if ($user->self_cert_form=='complete'){
                $self_cert_status = '<li><a href="javascript:void(0)" class="btn btn-xs btn-auto btn-success">Self Certified</a></li>';
            }

        }
        $return = ($email_status || $kyc_staus) ? '<div' . $g_id . ' class="user-account-status' . $g_cls . '">' . $heading . '<ul class="btn-grp">' . $email_status . $kyc_staus . $self_cert_status. '</ul></div>' : '';
        return $return;
    }
     ...
     * @return void
     */
    public static function user_select_token($data = null, $atttr = '')
    {
        $atttr_def = array('id' => '', 'class' => '', 'vers' => '');
        $opt_atttr = parse_args($atttr, $atttr_def);
        extract($opt_atttr);
        ...
        $return = '<ul' . $g_id . ' class="user-links bg-light' . $g_cls . '">
        <li><a href="' . route('user.token-list') . '">' . __('View Token offers') . '</a></li>
        </ul>
        <form id="select-token-form" action="' . route('user.token-list') . '" method="POST" style="display: none;"> <input type="hidden" name="_token" value="' . csrf_token() . '"> </form>';
    ...
    public static function content_block($data = null, $atttr = '')
    {
        ...
        if ($data == 'welcome') {
            ...
            $return .=' <p style="font-size: 15px;">Welcome to the Blockx Platform. To purchase tokens go through&nbsp;</span><a href="/user/contribute" style="background-color: rgb(255, 255, 255); font-size: 1em;">Buy Token</a><span style="color: rgb(73, 84, 99); font-size: 1em;">&nbsp;page.&nbsp;</span></p>';
            $return .=' <p style="font-size: 15px;">To learn more about BlockX, please visit <a href="https://blockxnet.com">https://blockxnet.com</a></p>';
            $return .= '</div>';
        ...
        if ($data == 'token_info') {
            ...
            if (!$currentToken)
                return (null);
            // available fields: 'name', 'token_symbol', 'short_description', 'description', 'url_more_info', 'logo'
            ...
            $return .= '<div class="card-content">';
            $return .= '<h4> Thank you for your interest in ' . $currentToken->name . " (".  $currentToken->token_symbol . ')</h4>';
            $return .= '<div><strong>';
            $return .= $currentToken->short_description;
            $return .= '</strong></div><br />';
    ...
    */
    public static function user_referral_info($data=null, $atttr='')
    {
        ...
        $heading    .= '<div class="card-head has-aside"><h6 class="card-title card-title-sm">'.__('Referral Link').'</h6>'.$more.'</div>';
