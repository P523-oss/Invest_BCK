...
class Transaction extends Model
{
    ...
    protected $fillable = ['tnx_id', 'token_id', 'tnx_type', 'tnx_time', 'tokens', 'bonus_on_base', 'bonus_on_token', 'total_bonus', 'total_tokens', 'stage', 'user', 'amount', 'receive_amount', 'receive_currency', 'base_amount', 'base_currency', 'base_currency_rate', 'currency', 'currency_rate', 'all_currency_rate', 'wallet_address', 'payment_method', 'payment_id', 'payment_to', 'checked_by', 'added_by', 'checked_time', 'details', 'extra', 'status', 'dist'
    ];
    ...
    */
    ...
    public static function dashboard($chart = 7) {
        ...
        $data['all'] = self::with('tnxToken')->whereNotIn('status', ['deleted', 'new'])->whereNotIn('tnx_type', ['withdraw'])->orderBy('created_at', 'DESC')->limit(4)->get();
     ...
     * @return void
     */
    public static function user_contribution($token_id=null)
    {
        if (is_null($token_id)) {
            $currentToken = unserialize(session('currenttoken'));
            $token_id =$currentToken->id;
        }
        $data = [];
        $curs = array_keys(PaymentMethod::Currency);
        $user_tnx = self::get_by_own(['token_id' => $token_id, 'status' => 'approved', 'tnx_type' => 'purchase', 'refund' => null])->get();
        foreach ($curs as $cur) {
    ...
    public static function user_mytoken($type='balance')
    {
        if($type=='balance') {
            ...
            $tub = $user->tableUserBalance();
            $balance_sum = (object) [
                // 'current'   => $user->tokenBalance,
                'current'   => 0,
                'tableUserBalance'   => $tub,
                // 'current_in_base' => token_price($user->tokenBalance, base_currency()),
                // 'current_in_base' => $user->sum_token_price(base_currency()),
                'current_in_base' => $user->getSumContributed(),
