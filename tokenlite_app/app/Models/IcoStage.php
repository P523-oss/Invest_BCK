...
class IcoStage extends Model
{
     ...
     * @return void
     */
    public static function dashboard($dashboardType="user", $token_id = 0)
    {
        if ($dashboardType=="user")
            $activeStage = active_stage();
        else
            $activeStage = active_stage($token_id);
        $tnxs = self::check_stage($activeStage->id);

        $data['stage'] = $activeStage;

        $data['trnxs'] = (object) [
            'all' => Transaction::where(['status' => 'approved', 'stage' => $activeStage->id])->count(),
            'pending' => Transaction::where(['status' => 'pending', 'stage' => $activeStage->id])->sum('total_tokens'),
            'percent' => to_percent($activeStage->soldout, $activeStage->total_tokens),
    ...
    public static function token_add_to_account($trnx, $stage_action = '', $user_action = '')
    {
        ...
        if ($user_action == 'add' || $user_action == 'sub') {
            $user->save();
            $user->addUserBalance($stage->token_id, $tnx_tokens, $tnx_amount, $user_action);
            return true;
        }
        return false;
    }
