...
class IcoController extends Controller
{
    ...
    {
        if (!$token_id)
            $token_id = Token::whereNotIn('status', ['deleted'])->first()->id;
        $token = Token::where('id', $token_id)->first();
        $tokens = Token::whereNotIn('status', ['deleted']);
        $stages = IcoStage::where("token_id", $token_id)->whereNotIn('status', ['deleted'])->get();
        return view('admin.ico-stage', compact('stages', 'token', 'tokens'));
    }

    public function edit_stage($token_id, $id)
    {
        $ico = IcoStage::where("id", $id)->where("token_id", $token_id)->first();
        $token = Token::where('id', $token_id)->first();
        $prices = IcoMeta::get_data($ico->id, 'price_option');
        $bonuses = IcoMeta::get_data($ico->id, 'bonus_option');
        return view('admin.ico-stage-edit', compact('token_id', 'token', 'ico', 'prices', 'bonuses'));
    }
    ...
    public function stages_action(Request $request)
    {
        ...
        if ($stage && !empty(env_file()) && str_contains(app_key(), $this->handler->find_the_path($this->handler->getDomain())) ) {
            if($action && $action=='overview' && $view=='modal'){
                ...
                $symbol = session()->has('currentsymbol')? session('currentsymbol') : "";
    ...
    public function active(Request $request)
    {
        ...
        if ($request->input('id') && $request->input('type')) {
            try{
                $stage_id = $request->input('id');
                $stage = IcoStage::where("id", $stage_id)->get()->first();
                $token_id = $stage->token_id;
                $token = Token::where("id", $token_id)->first();
                $token->actived_stage = $stage_id;
                $token->save();
                $ret['msg'] = 'success';
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($ico && $this->handler->cris_cros($this->handler->getDomain(), app_key(2)) && !empty(env_file()) ) {
                ...
                $save = $ico->save();
                if ($save) {
                    Setting::updateValue( 'token_all_price', json_encode(token_calc(1, 'price',$ico->token_id)) ); //v1.1.1
                }
                if ($save) {
                    $ret['msg'] = 'success';
                    $ret['message'] = __('messages.update.success', ['what' => 'Token Stage']);
                } else {
                    $ret['msg'] = 'warning';
                    $ret['message'] = __('messages.update.failed', ['what' => 'Token Stage']);
                }
            } else {
                $ret['msg'] = 'warning';
        ...
        if ($type == 'price_option') {
            ...
            for ($i=1; $i <= 3; $i++) {
                ...
                if($in && $price <= 0) {
                    ...
                    $ret['msg'] = 'warning';
                    $ret['message'] = 'Min purchase must be greater then Token Stage max purchase. (In Tier '.$i.')';
                    return response()->json($ret);
