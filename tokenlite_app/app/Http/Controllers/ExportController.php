...
class ExportController extends Controller
{
    ...
    protected function users(Request $request, $type='')
    {
        ...
        $order_by   = gmvl('user_order_by', 'id');
        ...
        if($type == 'entire'){
            ...
            $callback = function() use ($items, $head)
            {
                ...
                foreach($items as $item) {
                    // $item->tokenBalance, 
                    fputcsv($file, [$item->name, $item->email, $item->walletType, $item->walletAddress, $item->contributed, $item->status, $item->nationality, $item->mobile, $item->dateOfBirth, $item->referral, $item->registerMethod, $item->created_at]);
                }
                fclose($file);
            };
        } elseif($type == 'compact') {
            ...
            $callback = function() use ($items, $head)
            {
                ...
                foreach($items as $item) {
                    // $item->tokenBalance
                    fputcsv($file, [$item->name, $item->email, $item->walletAddress, ]);
                }
                fclose($file);
            };
        }else{
            ...
            $callback = function() use ($items, $head)
            {
                ...
                foreach($items as $item) {
                    // $item->tokenBalance, 
                    fputcsv($file, [$item->name, $item->email, $item->walletAddress, $item->contributed, $item->status]);
                }
                fclose($file);
            };
        }
