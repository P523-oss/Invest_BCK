...
class TokenController extends Controller
{
    ...
    public function index()
    {
        ...
        $stage_id = $token->actived_stage;
        $stage = IcoStage::where("id", $stage_id)->get()->first();

        if ($token->require_sign ==1){
            $signed = TokenUser::IsSigned($token->id, Auth::id());
            if (!$signed){
                return redirect()->route('user.signage');
            }
        }
        ...
        return view('user.token', compact('stage', 'currencies', 'bonus', 'bonus_amount', 'price', 'token_prices', 
            'is_price_show', 'minimum', 'active_bonus', 'pm_currency', 'contribution',
            'token', 'stage'));
    }
