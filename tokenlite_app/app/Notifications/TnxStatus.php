...
class TnxStatus extends Notification implements ShouldQueue
{
    ...
    {
        $symbol = Token::getSymbol($this->tnx_data->token_id);
        $shortcode =array(
            "\n",
            '[[token_name]]',
            '[[token_symbol]]',
        ...
        $replace = array(
            ...
            $this->tnx_data->total_tokens.' '.$symbol,
        );
    ...
    public function get_blade($name='', $data='')
    {
        ...
        if ($name == 'order_details') {
            ...
            $blade = '<table class="table order"><thead><th colspan="3">Order details are follows:</th></thead><tbody class="text-left"><tr><td width="150">Order ID</td><td width="15">:</td><td><strong>#'.$this->tnx_data->tnx_id.'</strong></td></tr><tr><td>Token Sale Stage</td><td>:</td><td><strong>'.$this->tnx_data->ico_stage->name.'</strong></td></tr><tr><td>Token Number</td><td>:</td><td><strong>'.$this->tnx_data->tokens.' '.$symbol.'</strong></td></tr><tr><td>Bonus </td><td>:</td><td><strong>'.$this->tnx_data->total_bonus.' '.$symbol.'</strong> </td></tr><tr><td>Total Token</td><td>:</td><td><strong>'.$this->tnx_data->total_tokens.' '.$symbol.'</strong> </td></tr><tr><td>Payment Amount</td><td>:</td><td><strong>'.$this->tnx_data->amount.' '.strtoupper($this->tnx_data->currency).'</strong></td></tr><tr><td>Payment Status</td><td>:</td><td><strong>'.ucfirst($this->tnx_data->status).'</strong></td></tr><tr><td>Payment Method</td><td>:</td><td><strong>'.ucfirst($this->tnx_data->payment_method).'</strong></td></tr>'.((!str_contains($this->template, 'admin') && ($this->tnx_data->status == 'pending' || $this->tnx_data->status == 'onhold')) ? $pay_address : '').'</tbody></table>';
        }
        return $blade;
    }
}
