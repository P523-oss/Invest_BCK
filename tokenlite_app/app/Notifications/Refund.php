...
class Refund extends Notification
{
    ...
    public function refund_details()
    {
        ...
        $table .= '<tr><td>Refund Token</td><td>:</td><td><strong>'.$this->refund->total_tokens.' '. $this->transaction->tnxToken->token_synbol .'</strong></td></tr>';
    ...
    {
        $if_bonus = ($this->transaction->total_bonus > 0) ? ' (included bonus '.$this->transaction->total_bonus.' '. $this->transaction->tnxToken->token_synbol .')' : '';

        $table = '<table class="table order">';
        $table .= '<thead><th colspan="3">Refund against order ('.$this->transaction->tnx_id.'):</th></thead>';
        $table .= '<tbody class="text-left">';
        
        $table .= '<tr><td width="150">Total Token</td><td width="15">:</td><td><strong>'.$this->transaction->total_tokens.' '. $this->transaction->tnxToken->token_synbol .'</strong>'.$if_bonus.'</td></tr>';
