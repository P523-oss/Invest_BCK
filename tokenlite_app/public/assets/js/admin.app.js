/*! TokenLite v1.1.5 | Copyright by Softnio. */ ! function (t) {
    ...
    t(document).on("click", "a.token-action", function (a) {
        a.preventDefault();
        var n = t(this).data("type"),
            o = "suspend_token" == n ? "warning" : "info",
            i = t(this).data("uid");
        "transactions" == n || "activities" == n || "referrals" == n ? (e.empty(), e.parent().find(".modal-backdrop").remove(), t.post(show_token_info, {
            uid: i,
            req_type: n,
            _token: csrf_token
        }).done(a => {
            a.status && "die" == a.status ? show_toast(a.msg, a.message, "ti ti-lock") : (bs_modal_hide(t(this)), e.html(a), e.children(".modal").modal("show"))
        })) : "suspend_token" != n && "active_token" != n && "reset_pwd" != n && "reset_2fa" != n || swal({
            title: "Are you sure?",
            icon: o,
            buttons: ["Cancel", "Yes"],
            dangerMode: !0
        }).then(a => {
            a && t.post(view_token_url, {
                uid: i,
                req_type: n,
                _token: csrf_token
            }).done(a => {
                null != a.msg && show_toast(a.msg, a.message), cl(a), ("active_token" == n || (n = "suspend_token")) && t(this).fadeOut(200, function () {
                    t(this).remove()
                }), t(document).find(".status_token").find(".badge").removeAttr("class").addClass("badge badge-" + a.css + " ucap"), t(".more-menu-" + i).append('<li><a href="#" data-uid="' + i + '" data-type="' + a.status + '" class="token-action"><em class="fas fa-ban"></em>' + ("suspend_token" == n ? "Active" : "Suspend") + "</a></li>"), e.html(a), e.children(".modal").modal("show")
            })
        })
    });
    ...
    t(document).on("click", ".tnx-action", function () {
        ...
        token = e.data("token") ? e.data("token") : 0, chk_adjust = e.data("_chk") ? e.data("_chk") : 0, adjusted_token = e.data("_adjusted") ? e.data("_adjusted") : 0, base_bonus = e.data("_b_bonus") ? e.data("_b_bonus") : 0, token_bonus = e.data("_t_bonus") ? e.data("_t_bonus") : 0, amount = e.data("_amount") ? e.data("_amount") : 0, swal_icon = "approved" == n ? "info" : "warning", swal_cta = "approved" == n ? "Approve" : "Yes", swal_ctac = "approved" == n ? "" : "danger", "approved" == n && null != amount && amount <= 0 ? show_toast("warning", "Invalid Received Amount!") : swal({
            title: "Are you sure?",
            text: "refund" == n ? "If ayou refund for this transactions, then the token will be added to the stage and subtract the token from user balance." : "",
            icon: swal_icon,
            buttons: {
                cancel: {
                    text: "Cancel",
